...
begin
  ...
  try
    if Assigned(Message)   then Message.DisposeOf;
    if Assigned(Animation) then Animation.DisposeOf;
    if Assigned(Arc)       then Arc.DisposeOf;
    if Assigned(Background)then Background.DisposeOf;
...
begin
  ...
  TPlatformServices.Current.SupportsPlatformService(
    IFMXVirtualKeyboardService, IInterface(LKeyboardSvc)
  );
